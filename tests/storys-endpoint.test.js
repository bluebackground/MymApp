...
const ROUTE = '/storys';
const SINGLE_ROUTE = '/storys/:storyID';
...
describe('API Endpoints for /storys', () => {
  ...
  describe(`${GET}  ${ROUTE}`, () => {
    // GET /storys
    it('should return MULTIPLE story objects as an array', function (done) {
      chai.request(server)
        .get(ROUTE)
        .send({
          "options": {}
  ...
  describe(`${GET}  ${SINGLE_ROUTE}`, () => {
    it(`should return a SINGLE story from ${GET} ${SINGLE_ROUTE}`, (done) => {
      ...
      chai.request(server)
        ...
        .end((error, response) => {
          ...
          if (objects.length > 0) {
            ...
            done(new Error('No storys were found in DB'));
          }
        });
    });
  ...
  describe(`${POST}  ${ROUTE}`, () => {
      ...
      chai.request(server)
        ...
        .then(function (res) {
          try {
            ...
            expect(object.title).to.equal(newStory.title);
            expect(object.title).to.be.a('string');
            expect(object.description).to.equal(newStory.description);
            expect(object.description).to.be.a('string');
            expect(object.creator).to.equal(newStory.creator.toHexString());
            expect(object.creator).to.be.a('string');
            expect(object.project).to.equal(newStory.project.toHexString());
            expect(object.project).to.be.a('string');
            expect(object.priority).to.equal(newStory.priority);
            expect(object.priority).to.be.a('number');
            expect(object.votes).to.be.a('array');
    ...
    it('should return MULTIPLE story objects, including newly added story', async function () {
      try {
