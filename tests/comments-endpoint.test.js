...
const SINGLE_ROUTE = '/comments/:commentID';
...
describe('API Endpoints for /comments', () => {
  ...
  beforeEach((done) => {
    ...
    Comment.remove({})
      .then(() => {
        Comment.insertMany(objectsArray);
      })
      .then(() => {
        done();
      })
  ...
  describe(`${GET}  ${ROUTE}`, () => {
    // GET /comments
    it('should return MULTIPLE comment objects as an array', function (done) {
      chai.request(server)
        .get(ROUTE)
        .send({
          "options": {}
  ...
  describe(`${GET}  ${SINGLE_ROUTE}`, () => {
    it(`should return a SINGLE comment from ${GET} ${SINGLE_ROUTE}`, (done) => {
      ...
      chai.request(server)
        ...
        .end((error, response) => {
          ...
          if (objects.length > 0) {
            ...
            done(new Error('No comments were found in DB'));
          }
        });
    });
  ...
  describe(`${POST}  ${ROUTE}`, () => {
      ...
      chai.request(server)
        ...
        .then(function (res) {
          try {
            ...
            expect(object.title).to.equal(newComment.title);
            expect(object.title).to.be.a('string');
            expect(object.description).to.equal(newComment.description);
            expect(object.description).to.be.a('string');
            expect(object.from).to.equal(newComment.from.toHexString());
            expect(object.from).to.be.a('string');
            expect(object.project).to.equal(newComment.project.toHexString());
            expect(object.project).to.be.a('string');
    ...
    it('should return MULTIPLE comment objects, including newly added comment', async function () {
      try {
